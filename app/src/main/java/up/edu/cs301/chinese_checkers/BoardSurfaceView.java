...
//        int j = 0;
        ...
        {
//            a = myXYs[i][0];
            for(int j = 0; j < 4; j++)//<= myXYs[0].length; j++)
//            if(j < 2)
            {
                for(int k = 0; k < 2; k++)
                {
                    if(cgs.intArray[i][0] != -2 || cgs.intArray[i][1] != -2)
                    {
//                    cgs.intArray[i][j];
                    /*myXYs really just has two columns, so you can just hardcode in 0 for the x coordinate
                    * and 1 for the y coordinate.*/
                        c.drawCircle(myXYs[i][0], myXYs[i][1], radius, player1Paint);
                    }
                }
            }
//            j++;
        }
//        c.drawCircle(myXYs[0][0], myXYs[0][1], radius, player1Paint);
